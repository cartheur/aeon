...
The library defines \texttt{OutputPresentation}, \texttt{IOutputAdapter}, and \texttt{OutputDispatcher}. A presentation contains speaker, text, requested modalities, optional emotive indication, and optional instructional displacement. The bundled \texttt{TerminalOutputAdapter} supports text. The runtime also includes an opt-in \texttt{SpeechOutputAdapter}: Windows uses its local SAPI voice through PowerShell, while Linux uses the self-contained \texttt{Cartheur.AeonVoice.Private} package. Tactile, gesture, animation, and hardware remain extension contracts rather than bundled device implementations.

\subsection{Speech output}

Speech is off by default. Set \texttt{voiceenabled} to \texttt{true} in \texttt{Aeon.Runtime/config/settings.xml}, then select \texttt{auto}, \texttt{windows-sapi}, or \texttt{aeonvoice} for \texttt{voicebackend}. \texttt{auto} selects SAPI on Windows and AeonVoice on Linux. Windows SAPI uses the operating system's default installed voice.

On Linux x64, the private package contains the managed AeonVoice wrapper, native libraries, English resource data, and the \texttt{Toptygin} and \texttt{Leena} voice profiles. Aeon's project selects the \texttt{linux-x64} runtime identifier automatically on Linux so these native libraries deploy beside the runtime. Aeon synthesizes a temporary WAV with the selected \texttt{voiceprofile}, then invokes \texttt{linuxaudioplayer} to play it. The default player is \texttt{aplay}; set an absolute player path if it is not available on \texttt{PATH}.
...
Each utterance is capped at 4,096 characters. The timeout is clamped to 1,000--120,000 milliseconds. Speech runs asynchronously, and synthesis, launch, timeout, or non-zero-exit failures are logged without ending the dialogue session. Enable it only after manually confirming that the selected profile and local player produce audio. The dispatcher likewise contains failures from any adapter so terminal output can continue.
