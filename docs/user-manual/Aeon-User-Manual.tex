...
The library defines \texttt{OutputPresentation}, \texttt{IOutputAdapter}, and \texttt{OutputDispatcher}. A presentation contains speaker, text, requested modalities, optional emotive indication, and optional instructional displacement. The bundled \texttt{TerminalOutputAdapter} supports only text. Voice, tactile, gesture, animation, and hardware are contracts for extension; they are not bundled device implementations.
...
The primary runtime settings are in \texttt{Aeon.Runtime/config/settings.xml}. Important entries include personality directories, language resources, timeout values, alone-state timing, terminal mode, and \texttt{emotiveequation}. During build, configuration and personality assets are copied beside the executable.
